        ...
        try {
            ...
            if (!Funcs::countryExist($cc)) {
                return Cache::remember('connect_country_err', 3600, function () use ($key) {
                    return ['size' => $key, 'data' => (json_encode(['status' => 0, 'message' => 'Unsupported country!']))];
                });
            }
            Funcs::addOnline($cc);
            ...
            if ($user) {
                ...
                if ($banned) {
                    return ['size' => $key, 'data' => (json_encode(['status' => -2, 'message' => $banned->reason]))];
                }
                if (strtolower($user->country) != $cc && env('BAN_CC_CHANGE') == 1) {
                    ...
                    return Cache::remember('connect_ban_cc', 3600, function () use ($key) {
                        return ['size' => $key, 'data' => (json_encode(['status' => -2, 'message' => "Auto ban CC"]))];
                    });
                }
                if (env('AUTO_BAN_MULTI') == 1) {
                    ...
                    return Cache::remember('connect_ban_multi', 3600, function () use ($key) {
                        return ['size' => $key, 'data' => (json_encode(['status' => -2, 'message' => "Banned for creating multiple accounts."]))];
                    });
                }
                $status = 1;
            }
            ...
            return ['size' => $key, 'data' => (json_encode($res))];
        } catch (\Exception $e) {
            return ['size' => $key, 'data' => (json_encode(['status' => 0, 'message' => $e->getMessage()]))];
            return Cache::remember('connect_general_err', 3600, function () use ($key) {
                return ['size' => $key, 'data' => (json_encode(['status' => 0, 'message' => 'Unexpected error occurred!']))];
            });
        }
    }
}
